  ...
  async function handleExportToExcel(dateRange = 'all') {
    try {
      const dataToExport = getPatientsByDateRange(dateRange).map((patient, index) => ({
        ...
        'cellNumber': patient.cellNumber || patient.contactNumber || '',
        'birthplace': patient.birthplace || '',
        'religion': patient.religion || '',
        'address': patient.address || '',
        'patientType': patient.patientType || 'visitor',
        'bloodType': patient.bloodType || '',
        'course': patient.course || patient.userId?.course || '',
        'yearLevel': patient.yearLevel || patient.userId?.yearLevel || '',
        'section': patient.section || patient.userId?.section || '',
        'department': patient.department || patient.userId?.department || '',
                        ...
                        <div key={index} className="visit-record" style={{ marginBottom: '1rem', padding: '1rem', backgroundColor: '#f8f9fa', borderRadius: '8px', borderLeft: '4px solid #e51d5e' }}>
                          ...
                          {visit.complaint && (
                            <div style={{ marginTop: '0.75rem', fontSize: '0.9rem' }}>
                              <span style={{ color: '#64748b' }}>Complaint:</span> <span style={{ color: '#1e293b' }}>{visit.complaint}</span>
                            </div>
                          )}
                          {visit.diagnosis && (
                            <div style={{ marginTop: '0.75rem', fontSize: '0.9rem' }}>
